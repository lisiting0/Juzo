...
export default {
  ...
  methods: {
    ...
    getTicketAgentDetail() {
      this.agentList = []
      ticketAgentDetail(this.agentListQuery.agentId).then(response => {
        if (response) {
    ...
    getAgentProducts() {
      ...
      setTimeout(() => {
        productAgentList({ agentId: this.selectedAgent[0].id }).then((response) => {
          this.loading = false
