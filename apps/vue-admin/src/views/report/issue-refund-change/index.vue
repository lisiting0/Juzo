      ...
      <el-row>
        ...
        <el-col :span="4">
          <el-form-item label="分销账号" prop="distributionAccount">
            <el-input v-model="listQuery.distributionAccount" clearable />
          </el-form-item>
        </el-col>
        <el-col :span="4">
...
import { issueRefundChangeTotal } from '@/api/report/report'
...
export default {
  ...
  mounted() {
    // this.paymentMethodOptions = Object.keys(
  ...
  methods: {
    getList() {
      this.loading = true
      issueRefundChangeTotal(this.listQuery).then((response) => {
        this.loading = false
        if (response.status === '200') {
          this.list = response.data.records
          this.total = response.data.total
          this.list.forEach((element) => {
            if (element.refundFinancialReport) {
              element.issueFinancialReport.businessMessage = '退票'
            } else if (element.changeFinancialReport) {
              element.issueFinancialReport.businessMessage = '改签'
            }
          })
        }
      })
