        ...
        <el-col :span="4">
          <el-form-item label="订单状态" prop="status">
            <el-select v-model="listQuery.orderStatus" clearable>
              <el-option
                v-for="env in refundStatusOptions"
                :key="env.value"
                :value="env.value"
                :label="env.label"
              />
...
import { refundFetchList } from '@/api/report/report'
...
export default {
  ...
  methods: {
    getList() {
      ...
      refundFetchList(this.listQuery).then((response) => {
        this.loading = false
        if (response) {
