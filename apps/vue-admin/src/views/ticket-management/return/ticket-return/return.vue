<template>
  <el-dialog width="50%" top="5vh" :visible.sync="dialogFormVisible" :title="'票证退回'">
    <el-form ref="formRef" :label-position="'right'" label-width="100px" :rules="rules" :model="dto">
      ...
      >
        ...
        <el-table-column align="center" label="退回数量" prop="count">
          <template slot-scope="scope">
            <el-input v-model="scope.row.count" clearable disabled placeholder="退回数量" />
          </template>
        </el-table-column>
      </el-table>
...
import { refundTicketProcess } from '@/api/ticket-manage/ticket-return'
  ...
  methods: {
    initReturnTickets(tickets) {
      ...
      for (var i = 0; i < this.dto.ticketProveStockBOList.length; i++) {
        ...
        this.dto.ticketProveStockBOList[i].count = this.dto.ticketProveStockBOList[i].surplusNum
      }
      // this.dto.ticketProveStockBOList = tickets
      // console.log('-------this.dto.ticketProveStockBOList', this.dto.ticketProveStockBOList)
    },
    handle() {
    ...
    onSave() {
      // this.dto.agentName = this.dto.agentId.agentName
      // this.dto.agentId = this.dto.agentId.id
      this.$refs['formRef'].validate(valid => {
        if (valid) {
          refundTicketProcess(this.dto).then(response => {
            if (response.status === '200') {
              ...
              this.$messge.error(response.message)
            }
          })
        }
      })
