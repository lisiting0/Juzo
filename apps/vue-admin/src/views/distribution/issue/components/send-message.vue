...
<template>
  <el-dialog width="70%" top="5vh" :visible.sync="dialogFormVisible" title="出票订单发送消息">
...
export default {
    ...
    methods: {
        ...
        handleOpen(detail) {
            ...
            this.ticketIssueOrderNo = detail.ticketIssueOrderNo
            this.orderPassengeList = detail.orderPassengerBOList.map(obj => ({ ...obj }))
        ...
        sendMessage() {
            ...
            Array.isArray(this.orderContactSelectedList) && this.orderContactSelectedList.length === 0) {
                ...
                this.messageData.orderNo = this.ticketIssueOrderNo
                ...
                this.$confirm('确定需要对该订单进行出票发送短信通知操作么？', '发送短信通知', {
                    ...
                    issueSuccessNotify(this.messageData).then((response) => {
        ...
        getMessageList() {
            // console.log()
            getSMSByOrderNo(this.ticketIssueOrderNo).then((response) => {
                if (response) {
