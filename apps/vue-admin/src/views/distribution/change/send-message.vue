...
<template>
  <el-dialog width="70%" top="5vh" :visible.sync="dialogFormVisible" title="改签订单发送消息">
...
import { changeSuccessNotify } from '@/api/agent/change-order'
import { getSMSByOrderNo } from '@/api/distribution/ticket-agent-order-notification'

export default {
    name: 'ChangeSendMessage',
    components: {
    },
    data() {
        return {
    ...
    methods: {
        ...
        handleOpen(detail) {
            ...
            this.orderPassengeList = detail.changeOrderPassengerBOList.map(obj => ({ ...obj }))
        ...
        sendMessage() {
            ...
            Array.isArray(this.orderContactSelectedList) && this.orderContactSelectedList.length === 0) {
                ...
                this.messageData.orderNo = this.changeOrderNo
                ...
                this.$confirm('确定需要对该订单进行出票发送短信通知操作么？', '发送短信通知', {
                  ...
                  changeSuccessNotify(this.messageData).then((response) => {
        ...
        getMessageList() {
            // console.log()
            getSMSByOrderNo(this.changeOrderNo).then((response) => {
                if (response) {
