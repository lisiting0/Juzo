<template>
  <el-dialog
    ...
    :title="dialogStatus == 'add' ? '供应商注册申请' : '供应商注册编辑'"
  >
    <el-form
      ref="formRef"
      :model="dto"
    ...
    >
      ...
      <div class="custom-card">
        ...
        <div class="card-box padding-2">
          <div align="center">
            <div class="card-item">
              <label class="card-item-title">{{
                dto.partnerType === 1 ? '公司名称' : '名称'
              }}</label>
              <div class="card-item-content">
...
import { createTicketSupplier } from '@/api/identity/ticket-supplier'
    ...
    createData() {
      this.$refs.formRef.validate(valid => {
        if (valid) {
          ...
          createTicketSupplier(this.dto).then(res => {
            this.loading = false
            if (res) {
              this.$notify({
              ...
              this.$confirm(`用户名：<span class='danger-weight-text'>${res.partnerName}</span><br/>账号：<span class='danger-weight-text'>${res.primaryUserName}</span><br/>密码：<span class='danger-weight-text'>${res.primaryUserPassword}</span>`, '注册的账号密码', {
                ...
                .then(() => {
                  return new Promise(resolve => {
                    this.$notify({
                      dangerouslyUseHTMLString: true,
                      title: '账号密码获取成功',
                      message: `用户名：<span class='danger-weight-text'>${res.partnerName}</span><br/>账号：<span class='danger-weight-text'>${res.primaryUserName}</span><br/>密码：<span class='danger-weight-text'>${res.primaryUserPassword}</span>`,
                      type: 'success',
                      duration: 10000
                    })
                    // 延迟一段时间后再关闭窗口
