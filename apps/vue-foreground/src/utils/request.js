...
const ExpiresTimeKey = 'Fore-Expires-Time'
const LoginTimeKey = 'Fore-Login-Time'
// 是否正在刷新的标记
...
const service = axios.create({
  ...
  timeout: 300000 // request timeout， 有些接口请求时间过长,先设置5分钟
})
...
service.interceptors.request.use(
  config => {
    ...
    if (store.getters.user) {
      config.headers['organization-id'] = store.getters.user.organizationId
      config.headers['organization-name'] = store.getters.userName
    }
...
service.interceptors.response.use(
  ...
  error => {
    ...
    if (error.response && error.response.data && error.response.data.error) {
      ...
      } else if (error.response.data.error.details != null) {
