<template>
  ...
  >
    ...
    >
      ...
      <el-form-item v-show="submitData.isVolunteer === 1" prop="fileList" label="上传附件">
        <el-upload
          class="upload-demo"
          action=""
          :auto-upload="false"
...
<script>
  ...
  import { refundOrderCreate, refundPreAmountQuery } from '@/api/domestic-tickets/refund-order'
  ...
  import { queryOrderInsurance } from '@/api/domestic-tickets/order-insurance'
  ...
  export default {
    ...
    mounted() {
      ...
      this.$refs.refundFlightDialogRef.rendered = true
    ...
    methods: {
      ...
      submit() {
        ...
        if (this.segmentTypeList.length === 1 && this.segmentTypeList[0] === 1) {
          this.$message.error('不符合航司退票条件(只退去程/第一程)')
          return
        }
