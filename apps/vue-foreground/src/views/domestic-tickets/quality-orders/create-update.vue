<template>
  ...
  >
    ...
    <div slot="footer" class="dialog-footer">
      <el-button type="primary" :loading="loading" @click="dialogStatus === 'add' ? createData() : updateData()">
        保存
      </el-button>
...
import { ticketAgentList } from '@/api/identity/agent'
import { qualityOrderCreate, qualityOrderUpdate } from '@/api/domestic-tickets/quality-orders'
        ...
        if (this.dto.appendixList.length > 0) {
          this.dto.appendixList.forEach(element => {
            if (element && element != null) {
              this.tempImages.push({
                url: element,
                uid: new Date() + ((Math.random() * 1000).toFixed(0))
              })
            }
          })
