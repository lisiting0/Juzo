      ...
      <div class="right-container">
        ...
        >
            ...
            <div style="text-align: right">
              <el-link type="primary" :underline="false" @click="navitoRegister()">分销商{{ $t("AbpAccount['Register']") }}</el-link>
            </div>
        </el-form>
      </div>
        ...
        this.$store
          ...
          .then(() => {
            this.$store.dispatch('app/applicationConfiguration').then((res) => {
              if (res.currentUser.roles.includes('TicketAgent')) {
                this.$router.push({
                  path: this.redirect || '/domestic-tickets/ticket-passenger',
                  query: this.otherQuery
                })
              } else {
                this.$message({
                  type: 'info',
                  message: `当前账号角色不是分销商，无法登录，请检查账号!`
                })
