<template>
  <el-dialog width="50%" :visible.sync="dialogFormVisible" :title="'票证退回'">
    <el-form ref="formRef" :label-position="'right'" label-width="100px" :rules="rules" :model="dto">
      ...
      >
        ...
        <el-table-column align="center" label="退回数量" prop="count">
          <template slot-scope="scope">
            <el-input v-model="scope.row.count" clearable placeholder="退回数量" />
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  import { agentProcessRefund } from '@/api/ticket/agent-ticketprove'
    ...
    methods: {
      ...
      onSave() {
        ...
        this.$refs['formRef'].validate(valid => {
          if (valid) {
            agentProcessRefund(this.dto).then(response => {
              if (response) {
